...
var range_1 = range.create;
var range_2 = range.isRange;
var range_3 = range.create2;
var range_4 = range.sort;
...
var _transport = {};
	...
	module.exports = function(obj) {
		if (obj) {
			common.extend(module.exports, obj);
		}
	};
...
module.exports = common.extend(module.exports, {
...
var stringStream = createCommonjsModule(function (module) {
/**
	 * @type StringStream
	 * @constructor
...
var stringStream_1 = stringStream.create;
...
var comments = {
		...
		strip: function(content) {
			...
			return common.replaceWith(content, replaceRanges, ' ');
		}
	};
...
var range$1 = range;
	 ...
	 */
	Range.prototype.extend = function(range$$1){
		// LeftPair, the right most range that l.start < range.start
		var l = this;
		if (l.start < range$$1.start) {
			while (l.next && l.next.start < range$$1.start) {
				l = l.next;
			}
		} else {
			while (l && l.start >= range$$1.start) {
				l = l.prev;
			}
		}
		// RightPair, the left most range that r.end > range.end
		var r = this;
		if (r.end > range$$1.end) {
			while (r.prev && r.prev.end > range$$1.end) {
				r = r.prev;
			}
		} else {
			while (r && r.end <= range$$1.end) {
				r = r.next;
			}
		}
	...
	Match.prototype.addExclude = function(range$$1){
		this.excludeRange.extend(range$$1);
	};
	...
	Tree.prototype.stackExclude = function(range$$1) {
		if (this.root && this.root.range.start >= range$$1.start && this.root.range.end <= range$$1.end) {
			this.finished = true;
			return;
		}
		...
		while (i) {
			tag = this.stack[i - 1];
			if (tag.range.start >= range$$1.end || tag.range.end <= range$$1.start) {
				break;
			}
			this.stack.pop();
			i--;
	...
	Search.prototype.next = function(){
		...
		if (this.explicit && !this.explicitPending) {
			...
			if (result.detached) {
				...
				this.explicit.matchGroup.matches[0].excludeRange.extend(range$$1);
				this.cleanTreeStack(range$$1);
			}
		}
		...
		while (i) {
			...
			if (result.detached) {
				...
				this.pool[name].matchGroup.matches[0].excludeRange.extend(range$$1);
				this.cleanTreeStack(range$$1);
			}
			i--;
		}
	...
	Search.prototype.cleanTreeStack = function(range$$1) {
		this.main.stackExclude(range$$1);
		if (this.explicit) {
			this.explicit.stackExclude(range$$1);
		}
		
		var i, len;
		for (i = 0, len = this.trees.length; i < len; i++) {
			this.pool[this.trees[i]].stackExclude(range$$1);
		}
	};
	...
	function createMatcher(text, pos) {
		...
		function buildExclude(i, j) {
			var range$$1 = excludeRange = new Range(pos, pos);
			while ((i = text.indexOf("<!--", i)) >= 0 && i < j) {
				...
				if (!tag) {
					...
					i = tag.range.end;
					range$$1 = range$$1.extend(new Range(
						tag.range.start,
						tag.range.end
					));
				}
...
var htmlMatcher = {
		...
		find: function(text, pos) {
			...
			if (open) {
				...
				if (close) {
					outerRange = range$1.create2(open.range.start, close.range.end);
					innerRange = range$1.create2(open.range.end, close.range.start);
				} else {
					outerRange = innerRange = range$1.create2(open.range.start, open.range.end);
				}
		...
		cachedFind: function(text, pos) {
			...
			if (!this._cache[hash]) {
				this._cache[hash] = this.uncachedFind(text, pos);
			}
...
/**
	...
	function inherits(parent, protoProps, staticProps) {
		...
		// if supplied.
		if (protoProps)
			common.extend(child.prototype, protoProps);

		// Add static properties to the constructor function, if supplied.
		if (staticProps)
			common.extend(child, staticProps);
...
/**
	...
	function EditContainer(source, options) {
		this.options = common.extend({offset: 0}, options);
		/**
	...
	EditContainer.prototype = {
		...
		_updateSource: function(value, start, end) {
			...
			this.source = common.replaceSubstring(this.source, value, r);
		},
		...
		get: function(name) {
			...
			if (typeof name === 'string') {
				return common.find(this.list(), function(prop) {
		...
		itemFromPosition: function(pos, isAbsolute) {
			return common.find(this.list(), function(elem) {
...
var defaultOptions = {
	...
	var XMLEditContainer = base.EditContainer.extend({
		initialize: function(source, options) {
			common.defaults(this.options, defaultOptions);
			this._positions.name = 1;
			
			var attrToken = null;
			var tokens = xml.parse(source);
		...
		add: function(name, value, pos) {
			var list = this.list();
			var start = this.nameRange().end;
			var styles = common.pick(this.options, 'styleBefore', 'styleSeparator', 'styleQuote');
			...
			if (donor) {
				styles = common.pick(donor, 'styleBefore', 'styleSeparator', 'styleQuote');
			}
			
			value = styles.styleQuote + value + styles.styleQuote;
			
			var attribute = new XMLEditElement(this, 
					base.createToken(start + styles.styleBefore.length, name),
					base.createToken(start + styles.styleBefore.length + name.length 
							+ styles.styleSeparator.length, value)
					);
			
			common.extend(attribute, styles);
		...
		removeClass: function(value) {
			...
			var reClass = new RegExp('(^|\\s+)' + common.escapeForRegexp(value));
			var classVal = attr.value().replace(reClass, '');
			if (!common.trim(classVal)) {
				this.remove('class');
			} else {
				attr.value(classVal);
			}
	...
	var XMLEditElement = base.EditElement.extend({
...
var idCounter = 1;
	...
	CSSSection.prototype = {
		addChild: function(section) {
			...
			var lastChild = common.last(this.children);
			if (lastChild) {
		...
		name: function() {
			if (this._name === null) {
				var range$$1 = this.nameRange();
				if (range$$1) {
					this._name = range$$1.substring(this.source());
				}
			}
		...
		content: function() {
			...
			return this._content = common.trim(out);
		}
	};
...
var cssSections = {
			...
			while (ch = stream.next()) {
				...
				if (ch == '{') {
					...
					if (matchedRanges.length) {
						stream.pos = common.last(matchedRanges).end;
						continue;
					} 
				}
			}
		...
		sanitize: function(content) {
			content = comments.strip(content);
			...
			return common.replaceWith(content, replaceRanges, 'a');
		},
		...
		sectionTree: function(content) {
			...
			var insert = function(range$$1, ctx) {
				while (ctx && ctx.range) {
					if (ctx.range.contains(range$$1)) {
						return ctx.addChild(range$$1);
					}
		...
		styleAttrRange: function(content, pos) {
			var tree = xml$1.parseFromPosition(content, pos, true);
			if (tree) {
...
/**
	...
	HandlerList.prototype = {
		...
		add: function(fn, options) {
			...
			this._list.push(common.extend({}, options, {order: order, fn: fn}));
		},
		 ...
		 */
		remove: function(fn) {
			var item = common.find(this._list, function(item) {
				return item.fn === fn;
			});
			if (item) {
		...
		exec: function(skipValue, args) {
			args = args || [];
			var result = null;
			common.find(this.list(), function(h) {
...
var elements = createCommonjsModule(function (module) {
var factories = {};
...
var elements_1 = elements.add;
var elements_2 = elements.get;
var elements_3 = elements.create;
var elements_4 = elements.is;
var elements_5 = elements.type;

var logger = createCommonjsModule(function (module, exports) {
/**
 * Simple logger for Emmet
 */
if ('object' === 'object' && typeof define !== 'function') {
...
var preferences_1 = {
		...
		exportModified: function() {
			return common.extend({}, preferences);
		},
...
preferences_1.define('caniuse.enabled', true, 'Enable support of Can I Use database. When enabled,\
		CSS abbreviation resolver will look at Can I Use database first before detecting\
		CSS properties that should be resolved');

	preferences_1.define('caniuse.vendors', 'all', 'A comma-separated list vendor identifiers\
	...
	preferences_1.define('caniuse.era', 'e-2', 'Browser era, as defined in Can I Use database.\
	...
	function parseCSS(data) {
		...
		Object.keys(data.data).forEach(function(name) {
			...
			if (name in cssSections$1) {
				cssSections$1[name].forEach(function(kw) {
	...
	function getVendorsList() {
		...
		var vendors = preferences_1.getArray('caniuse.vendors');
	...
	function getVersionSlice() {
		var era = preferences_1.get('caniuse.era');
...
var caniuse = {
		...
		resolvePrefixes: function(property) {
			if (!preferences_1.get('caniuse.enabled') || !cssDB || !(property in cssDB)) {
				return null;
			}
			...
			return common.unique(prefixes).sort(function(a, b) {
...
function parseArgs(str) {
		...
		while (!stream.eol()) {
			if (stream.peek() == ',') {
				args.push(common.trim(stream.current()));
				stream.next();
				stream.start = stream.pos;
			}
			stream.next();
...
var defaultOptions$1 = {
	...
	function findParts(str) {
		...
		return common.unique(result.filter(function(item) {
			return !!item.length();
		}));
	}
	...
	function consumeProperties(node, source, offset) {
		...
		list.forEach(function(property) {
			node._children.push(new CSSEditElement(node,
				base.createToken(property.name.start, property.nameText),
				base.createToken(property.value.start, property.valueText),
				base.createToken(property.end.start, property.endText)
				));
		});
	}
	 ...
	 */
	var CSSEditContainer = base.EditContainer.extend({
		initialize: function(source, options) {
			common.extend(this.options, defaultOptions$1, options);
			
			if (Array.isArray(source)) {
				source = css.toSource(source);
			}
			...
			allRules.forEach(function(r) {
				var isTopLevel = !common.find(topLevelRules, function(tr) {
			...
			topLevelRules.forEach(function(r) {
				...
				var opt = common.extend({}, that.options, {offset: r.start + that.options.offset});
		...
		_saveStyle: function() {
			...
			this.list().forEach(function(p) {
				...
				var lines = common.splitByLines(p.styleBefore);
				if (lines.length > 1) {
		...
		add: function(name, value, pos) {
			var list = this.list();
			var start = this._positions.contentStart;
			var styles = common.pick(this.options, 'styleBefore', 'styleSeparator');
			...
			if (donor) {
				styles = common.pick(donor, 'styleBefore', 'styleSeparator');
			}
			
			var nameToken = base.createToken(start + styles.styleBefore.length, name);
			var valueToken = base.createToken(nameToken.end + styles.styleSeparator.length, value);
			
			var property = new CSSEditElement(this, nameToken, valueToken,
					base.createToken(valueToken.end, ';'));
			
			common.extend(property, styles);
	 ...
	 * @constructor
	 */
	var CSSEditElement = base.EditElement.extend({
		...
		value: function(val) {
			...
			if (isUpdating && this.isIncomplete()) {
				var self = this;
				var donor = common.find(allItems, function(item) {
...
var css$1 = {
		 ...
		 */
		propertyFromPosition: function(css$$1, pos) {
			var cssProp = null;
			/** @type EditContainer */
			var cssRule = typeof css$$1 === 'string' ? this.parseFromPosition(css$$1, pos, true) : css$$1;
			if (cssRule) {
				cssProp = cssRule.itemFromPosition(pos, true);
				if (!cssProp) {
					// in case user just started writing CSS property
					// and didn't include semicolon–try another approach
					cssProp = common.find(cssRule.list(), function(elem) {
...
var css$2 = createCommonjsModule(function (module) {
var prefixObj = {
	...
	preferences_1.define('css.valueSeparator', ': ',
			'Defines a symbol that should be placed between CSS property and ' 
			+ 'value when expanding CSS abbreviations.');
	preferences_1.define('css.propertyEnd', ';',
			'Defines a symbol that should be placed at the end of CSS property  ' 
			+ 'when expanding CSS abbreviations.');
	
	preferences_1.define('stylus.valueSeparator', ' ',
			'Defines a symbol that should be placed between CSS property and ' 
			+ 'value when expanding CSS abbreviations in Stylus dialect.');
	preferences_1.define('stylus.propertyEnd', '',
			'Defines a symbol that should be placed at the end of CSS property  ' 
			+ 'when expanding CSS abbreviations in Stylus dialect.');
	
	preferences_1.define('sass.propertyEnd', '',
			'Defines a symbol that should be placed at the end of CSS property  ' 
			+ 'when expanding CSS abbreviations in SASS dialect.');

	preferences_1.define('css.syntaxes', 'css, less, sass, scss, stylus, styl',
			'List of syntaxes that should be treated as CSS dialects.');
	
	preferences_1.define('css.autoInsertVendorPrefixes', true,
	...
	preferences_1.define('less.autoInsertVendorPrefixes', false, 'Same as <code>css.autoInsertVendorPrefixes</code> but for LESS syntax');
	preferences_1.define('scss.autoInsertVendorPrefixes', false, 'Same as <code>css.autoInsertVendorPrefixes</code> but for SCSS syntax');
	preferences_1.define('sass.autoInsertVendorPrefixes', false, 'Same as <code>css.autoInsertVendorPrefixes</code> but for SASS syntax');
	preferences_1.define('stylus.autoInsertVendorPrefixes', false, 'Same as <code>css.autoInsertVendorPrefixes</code> but for Stylus syntax');
	...
	Object.keys(props).forEach(function(k) {
		preferences_1.define('css.' + k + 'Properties', props[k], descTemplate({vendor: k}));
		preferences_1.define('css.' + k + 'PropertiesAddon', '', descAddonTemplate({vendor: k}));
	});
	
	preferences_1.define('css.unitlessProperties', 'z-index, line-height, opacity, font-weight, zoom', 
			'The list of properties whose values ​​must not contain units.');
	
	preferences_1.define('css.intUnit', 'px', 'Default unit for integer values');
	preferences_1.define('css.floatUnit', 'em', 'Default unit for float values');
	
	preferences_1.define('css.keywords', 'auto, inherit, all', 
			'A comma-separated list of valid keywords that can be used in CSS abbreviations.');
	
	preferences_1.define('css.keywordAliases', 'a:auto, i:inherit, s:solid, da:dashed, do:dotted, t:transparent', 
			'A comma-separated list of keyword aliases, used in CSS abbreviation. '
			+ 'Each alias should be defined as <code>alias:keyword_name</code>.');
	
	preferences_1.define('css.unitAliases', 'e:em, p:%, x:ex, r:rem', 
			'A comma-separated list of unit aliases, used in CSS abbreviation. '
			+ 'Each alias should be defined as <code>alias:unit_value</code>.');
	
	preferences_1.define('css.color.short', true, 
			'Should color values like <code>#ffffff</code> be shortened to '
			+ '<code>#fff</code> after abbreviation with color was expanded.');
	
	preferences_1.define('css.color.case', 'keep', 
			'Letter case of color values generated by abbreviations with color '
			+ '(like <code>c#0</code>). Possible values are <code>upper</code>, '
			+ '<code>lower</code> and <code>keep</code>.');
	
	preferences_1.define('css.fuzzySearch', true, 
	...
	preferences_1.define('css.fuzzySearchMinScore', 0.3, 
			'The minium score (from 0 to 1) that fuzzy-matched abbreviation should ' 
			+ 'achive. Lower values may produce many false-positive matches, '
			+ 'higher values may reduce possible matches.');
	
	preferences_1.define('css.alignVendor', false, 
	...
	function normalizeValue(value) {
		...
		if (ch == '$') {
			return common.escapeText(value);
		}
	...
	function normalizeHexColor(value) {
		...
		var repeat = common.repeatString;
		var color = null;
		switch (hex.length) {
			case 1:
				color = repeat(hex, 6);
		...
		if (preferences_1.get('css.color.short')) {
			var p = color.split('');
		...
		switch (preferences_1.get('css.color.case')) {
			case 'upper':
	...
	function getKeyword(name) {
		var aliases = preferences_1.getDict('css.keywordAliases');
		return name in aliases ? aliases[name] : name;
	}
	
	function getUnit(name) {
		var aliases = preferences_1.getDict('css.unitAliases');
		return name in aliases ? aliases[name] : name;
	}
	
	function isValidKeyword(keyword) {
		return ~preferences_1.getArray('css.keywords').indexOf(getKeyword(keyword));
	}
	...
	function hasPrefix(property, prefix) {
		...
		if (!info)
			info = common.find(vendorPrefixes, function(data) {
	...
	function findVendorPrefixes(property) {
		var prefixes = caniuse.resolvePrefixes(property);
		if (!prefixes) {
	...
	function addPrefix(name, obj) {
		...
		vendorPrefixes[name] = common.extend({}, prefixObj, obj);
	}
	...
	function getSyntaxPreference(name, syntax) {
		if (syntax) {
			...
			var val = preferences_1.get(syntax + '.' + name);
			if (typeof val !== 'undefined') {
				return val;
			}
		}
		
		return preferences_1.get('css.' + name);
	}
	...
	function formatProperty(property, syntax) {
		...
		property = property.substring(0, ix).replace(/\s+$/, '') 
			...
			+ common.trim(property.substring(ix + 1));
	...
	function getProperties(key) {
		var list = preferences_1.getArray(key);
		var addon = preferences_1.getArray(key + 'Addon');
		if (addon) {
			addon.forEach(function(prop) {
				if (prop.charAt(0) == '-') {
					list = common.without(list, prop.substr(1));
				} else {
					if (prop.charAt(0) == '+')
						prop = prop.substr(1);
	...
	function resolvePrefixedValues(snippetObj, isImportant, syntax) {
		...
		var parts = css$1.findParts(snippetObj.value);
		parts.reverse();
		parts.forEach(function(p) {
			...
			(findVendorPrefixes(partValue) || []).forEach(function(prefix) {
				...
				lookup[prefix] = common.replaceSubstring(lookup[prefix], '-' + prefix + '-' + partValue, p);
			});
		});
	...
	module.exports = {
		...
		resolve: function(node, syntax) {
			var cssSyntaxes = preferences_1.getArray('css.syntaxes');
		...
		parseValues: function(str) {
			...
			while ((ch = stream.next())) {
				if (ch == '$') {
					...
					if (isValidKeyword(common.last(values)) || 
		...
		normalizeValue: function(value, property) {
			...
			var unitlessProps = preferences_1.getArray('css.unitlessProperties');
			return value.replace(/^(\-?[0-9\.]+)([a-z]*)$/, function(str, val, unit) {
				if (!unit && (val == '0' || ~unitlessProps.indexOf(property)))
					return val;
				
				if (!unit)
					return val.replace(/\.$/, '') + preferences_1.get(~val.indexOf('.') ? 'css.floatUnit' : 'css.intUnit');
		...
		expand: function(abbr, value, syntax) {
			syntax = syntax || 'css';
			var autoInsertPrefixes = preferences_1.get(syntax + '.autoInsertVendorPrefixes');
			...
			var abbrData = common.extend(prefixData, valuesData);
			...
			if (!snippet && preferences_1.get('css.fuzzySearch')) {
				// let’s try fuzzy search
				snippet = resources.fuzzyFindSnippet(syntax, abbrData.property, parseFloat(preferences_1.get('css.fuzzySearchMinScore')));
			}
			...
			if (preferences_1.get('css.alignVendor')) {
				var pads = common.getStringsPads(names);
		...
		splitSnippet: function(snippet) {
			...
			return {
				name: common.trim(pair.shift()),
				// replace ${0} tabstop to produce valid vendor-prefixed values
				// where possible
				value: common.trim(pair.join(':')).replace(/^(\$\{0\}|\$0)(\s*;?)$/, '${1}$2')
			};
		},
...
var css_1$2 = css$2.addPrefix;
var css_2$2 = css$2.supportsPrefix;
var css_3$2 = css$2.resolve;
var css_4$2 = css$2.prefixed;
var css_5$1 = css$2.listPrefixes;
var css_6$1 = css$2.getPrefix;
var css_7 = css$2.removePrefix;
var css_8 = css$2.extractPrefixes;
var css_9 = css$2.findValuesInAbbreviation;
var css_10 = css$2.parseValues;
var css_11 = css$2.extractValues;
var css_12 = css$2.normalizeValue;
var css_13 = css$2.expand;
var css_14 = css$2.expandToSnippet;
var css_15 = css$2.splitSnippet;
var css_16 = css$2.getSyntaxPreference;
var css_17 = css$2.transformSnippet;
var css_18 = css$2.vendorPrefixes;
...
var VOC_SYSTEM = 'system';
	...
	function normalizeCaretPlaceholder(text) {
		return common.replaceUnescapedSymbol(text, '|', common.getCaretPlaceholder());
	}
	...
	common.extend(exports, {
		...
		getMatchedResource: function(node, syntax) {
			return resolvers.exec(null, common.toArray(arguments))
		...
		getSection: function(name) {
			...
			if (!(name in cache)) {
				cache[name] = common.deepMerge({}, systemSettings[name], userSettings[name]);
			}

			var data = cache[name], subsections = common.toArray(arguments, 1), key;
		...
		getAllSnippets: function(syntax) {
			...
			if (!cache[cacheKey]) {
				...
				cache[cacheKey] = common.extend.apply(common, stack.reverse());
			}
	...
	exports.addResolver(css$2.resolve.bind(css$2));
...
var tabStops = {
		...
		extract: function(text, options) {
			...
			options = common.extend({}, defaultOptions$2, options, {
				tabstop: function(data) {
			...
			if (options.replaceCarets) {
				text = text.replace(new RegExp( common.escapeForRegexp( common.getCaretPlaceholder() ), 'g'), '${0:cursor}');
			}
		...
		variablesResolver: function(node) {
			...
			return function(str, varName) {
				...
				if (varName == 'cursor') {
					return common.getCaretPlaceholder();
				}
...
preferences_1.define('profile.allowCompactBoolean', true, 
	...
	preferences_1.define('profile.booleanAttributes', '^contenteditable|seamless|async|autofocus|autoplay|checked|controls|defer|disabled|formnovalidate|hidden|ismap|loop|multiple|muted|novalidate|readonly|required|reversed|selected|typemustmatch$', 
	...
	function OutputProfile(options) {
		common.extend(this, defaultProfile, options);
	}
	...
	OutputProfile.prototype = {
		 ...
		 */
		cursor: function() {
			return this.place_cursor ? common.getCaretPlaceholder() : '';
		},
		...
		isBoolean: function(name, value) {
			...
			var boolAttrs = preferences_1.get('profile.booleanAttributes');
			if (!value && boolAttrs) {
		...
		allowCompactBoolean: function() {
			return this.compact_bool && preferences_1.get('profile.allowCompactBoolean');
		}
	};
...
var profile = {
		...
		create: function(name, options) {
			if (arguments.length == 2)
				...
				return new OutputProfile(common.defaults(name || {}, defaultProfile));
		},
...
var tagName$1 = {
		...
		removeElementFromCollection: function(name, collection) {
			if (collection in elementTypes) {
				elementTypes[collection] = common.without(this.getCollection(collection), name);
			}
		},
...
var abbreviation = {
		...
		isInline: function(node) {
			return node.isTextNode() 
				|| !node.name() 
				|| tagName$1.isInlineLevel(node.name());
		},
		...
		hasTagsInContent: function(node) {
			return common.matchesTag(node.content);
		},
		...
		insertChildContent: function(text, childContent, options) {
			...
			text = tabStops.replaceVariables(text, function(variable, name, data) {
				...
				if (name == 'child') {
					...
					output = common.padString(childContent, common.getLinePaddingFromPosition(text, data.start));
...
preferences_1.define('format.noIndentTags', 'html', 
			'A comma-separated list of tag names that should not get inner indentation.');
	
	preferences_1.define('format.forceIndentationForTags', 'body', 
	...
	function getIndentation(node) {
		var items = preferences_1.getArray('format.noIndentTags') || [];
	...
	function hasBlockSibling(item) {
		return item.parent && abbreviation.hasBlockChildren(item.parent);
	}
	...
	function shouldAddLineBreak(node, profile) {
		if (profile.tag_nl === true || abbreviation.isBlock(node))
			return true;
	...
	function shouldFormatInline(node, profile) {
		var nodeCount = 0;
		return !!common.find(node.children, function(child) {
			if (child.isTextNode() || !abbreviation.isInline(child))
				nodeCount = 0;
			else if (abbreviation.isInline(child))
				nodeCount++;
	...
	function processSnippet(item, profile) {
		...
		if (!isVeryFirstChild(item) && profile.tag_nl !== false && shouldAddLineBreak(item, profile)) {
			...
			if (isRoot(item.parent) || !abbreviation.isInline(item.parent)) {
				item.start = '\n' + item.start;
			}
		}
	...
	function shouldBreakInsideInline(node, profile) {
		var hasBlockElems = node.children.some(function(child) {
			if (abbreviation.isSnippet(child))
				return false;
			
			return !abbreviation.isInline(child);
		});
	...
	function processTag(item, profile) {
		...
		var isUnary = abbreviation.isUnary(item);
		var nl = '\n';
		...
		if (profile.tag_nl !== false) {
			...
			if (!forceNl) {
				var forceIndentTags = preferences_1.getArray('format.forceIndentationForTags') || [];
			...
			if (!item.isTextNode()) {
				if (shouldAddLineBreak(item, profile)) {
					...
					if (!isVeryFirstChild(item) && (!abbreviation.isSnippet(item.parent) || item.index()))
						item.start = nl + item.start;
						
					if (abbreviation.hasBlockChildren(item) || shouldBreakChild(item, profile) || (forceNl && !isUnary))
						item.end = nl + item.end;
						
					if (abbreviation.hasTagsInContent(item) || (forceNl && !item.children.length && !isUnary))
						item.start += nl + indent;
				} else if (abbreviation.isInline(item) && hasBlockSibling(item) && !isVeryFirstChild(item)) {
					item.start = nl + item.start;
				} else if (abbreviation.isInline(item) && shouldBreakInsideInline(item, profile)) {
					item.end = nl + item.end;
				}
	 ...
	 */
var format = function process(tree, profile, level) {
		level = level || 0;
	
		tree.children.forEach(function(item) {
			if (abbreviation.isSnippet(item)) {
...
/**
	 ...
	 * @param {OutputProfile} profile
	 */
	function processTag$1(item, profile) {
		...
		item.start = common.replaceSubstring(item.start, start, item.start.indexOf(placeholder), placeholder);
		item.end = common.replaceSubstring(item.end, end, item.end.indexOf(placeholder), placeholder);
...
var html = function process(tree, profile, level) {
		level = level || 0;
		
		if (!level) {
			tree = format(tree, profile, level);
		}
		
		tree.children.forEach(function(item) {
			if (!abbreviation.isSnippet(item)) {
				processTag$1(item, profile, level);
			}
...
function transformClassName(className) {
		return common.trim(className).replace(/\s+/g, '.');
	}
	...
	function condenseDataAttrs(attrs) {
		...
		attrs.forEach(function(attr) {
			if (reData.test(attr.name)) {
				...
				data.push(common.extend({}, attr, {name: attr.name.replace(reData, '')}));
			} else {
				out.push(attr);
			}
		});
	 ...
	 */
	function makeAttributesString$1(tag, profile) {
		var attrs = '';
		var otherAttrs = [];
		var attrQuote = profile.attributeQuote();
		var cursor = profile.cursor();
	 ...
	 * @param {OutputProfile} profile
	 */
	function processTag$2(item, profile) {
		if (!item.parent)
			// looks like it's root element
			return item;
		
		var attrs = makeAttributesString$1(item, profile);
		var cursor = profile.cursor();
		var isUnary = abbreviation.isUnary(item);
...
var haml = function process(tree, profile, level) {
		level = level || 0;
		
		if (!level) {
			tree = format(tree, '_format', profile);
		}
		
		tree.children.forEach(function(item) {
			if (!abbreviation.isSnippet(item)) {
				processTag$2(item, profile, level);
			}
...
var reNl = /[\n\r]/;
	...
	function transformClassName$1(className) {
		return common.trim(className).replace(/\s+/g, '.');
	}

	function stringifyAttrs$1(attrs, profile$$1) {
		var attrQuote = profile$$1.attributeQuote();
		return '(' + attrs.map(function(attr) {
			if (attr.isBoolean) {
				return attr.name;
			}
	 ...
	 * @param {Object} profile
	 */
	function makeAttributesString$2(tag, profile$$1) {
		var attrs = '';
		var otherAttrs = [];
		var attrQuote = profile$$1.attributeQuote();
		var cursor = profile$$1.cursor();
		
		tag.attributeList().forEach(function(a) {
			var attrName = profile$$1.attributeName(a.name);
			switch (attrName.toLowerCase()) {
				// use short notation for ID and CLASS attributes
				case 'id':
					attrs += '#' + (a.value || cursor);
					break;
				case 'class':
					attrs += '.' + transformClassName$1(a.value || cursor);
					break;
				// process other attributes
				default:
					otherAttrs.push({
						name: attrName,
						value: a.value || cursor,
						isBoolean: profile$$1.isBoolean(a.name, a.value)
					});
			}
		});
		
		if (otherAttrs.length) {
			attrs += stringifyAttrs$1(otherAttrs, profile$$1);
		}
	...
	function processTagContent(item) {
		...
		if (reNl.test(content) && !reIndentedText.test(content)) {
			...
			var pad = '| ';
			item.content = '\n' + pad + common.padString(content, pad);
		} else if (!reSpace$1.test(content)) {
	 ...
	 * @param {OutputProfile} profile
	 */
	function processTag$3(item, profile$$1) {
		if (!item.parent)
			// looks like it's a root (empty) element
			return item;
		
		var attrs = makeAttributesString$2(item, profile$$1);
		var cursor = profile$$1.cursor();
		var isUnary = abbreviation.isUnary(item);
			
		// define tag name
		var tagName = profile$$1.tagName(item.name());
...
var jade = function process(tree, curProfile, level) {
		level = level || 0;
		
		if (!level) {
			// always format with `xml` profile since
			// Jade requires all tags to be on separate lines
			tree = format(tree, profile.get('xml'));
		}
		
		tree.children.forEach(function(item) {
			if (!abbreviation.isSnippet(item)) {
				processTag$3(item, curProfile, level);
			}
...
var reNl$1 = /[\n\r]/;
	var reIndentedText$1 = /^\s*\|/;
	var reSpace$2 = /^\s/;

	preferences_1.define('slim.attributesWrapper', 'none', 
	...
	function transformClassName$2(className) {
		return common.trim(className).replace(/\s+/g, '.');
	}

	function getAttrWrapper() {
		var start = ' ', end = '';
		switch (preferences_1.get('slim.attributesWrapper')) {
			case 'round':
				start = '(';
				end = ')';
				break;
	...
	function stringifyAttrs$2(attrs, profile$$1) {
		var attrQuote = profile$$1.attributeQuote();
	 ...
	 * @param {Object} profile
	 */
	function makeAttributesString$3(tag, profile$$1) {
		var attrs = '';
		var otherAttrs = [];
		var attrQuote = profile$$1.attributeQuote();
		var cursor = profile$$1.cursor();
		...
		tag.attributeList().forEach(function(a) {
			...
			switch (attrName.toLowerCase()) {
				...
				case 'id':
					...
					break;
				case 'class':
					attrs += '.' + transformClassName$2(a.value || cursor);
					break;
				// process other attributes
				default:
					otherAttrs.push({
		...
		if (otherAttrs.length) {
			attrs += stringifyAttrs$2(otherAttrs, profile$$1);
		}
		
		return attrs;
	}

	function processTagContent$1(item) {
		if (!item.content) {
			return;
		}
		...
		if (reNl$1.test(content) && !reIndentedText$1.test(content)) {
			// multiline content: pad it with indentation and pipe
			var pad = '  ';
			item.content = '\n| ' + common.padString(content, pad);
		} else if (!reSpace$2.test(content)) {
			item.content = ' ' + content;
		}
	}
	 ...
	 * @param {OutputProfile} profile
	 */
	function processTag$4(item, profile$$1) {
		if (!item.parent)
			// looks like it's a root (empty) element
			return item;
		
		var attrs = makeAttributesString$3(item, profile$$1);
		var cursor = profile$$1.cursor();
		var isUnary = abbreviation.isUnary(item);
		var selfClosing = profile$$1.self_closing_tag && isUnary ? '/' : '';
...
var slim = function process(tree, curProfile, level) {
		level = level || 0;
		...
		tree.children.forEach(function(item) {
			if (!abbreviation.isSnippet(item)) {
				processTag$4(item, curProfile, level);
			}
...
/**
	 ...
	 * @param {AbbreviationNode} item
	 */
	function isVeryFirstChild$1(item) {
		return item.parent && !item.parent.parent && !item.index();
	}

var css$3 = function process(tree, profile, level) {
		level = level || 0;
		
		tree.children.forEach(function(item) {
			if (!isVeryFirstChild$1(item) && profile.tag_nl !== false) {
				item.start = '\n' + item.start;
			}
			process(item, profile, level + 1);
		});
...
preferences_1.define('bem.elementSeparator', '__', 'Class name’s element separator.');
	preferences_1.define('bem.modifierSeparator', '_', 'Class name’s modifier separator.');
	preferences_1.define('bem.shortElementPrefix', '-', 
	...
	function getSeparators() {
		return {
			element: preferences_1.get('bem.elementSeparator'),
			modifier: preferences_1.get('bem.modifierSeparator')
		};
	}
	...
	function bemParse(item) {
		...
		item.__bem.block = common.find(classNames, function(name) {
		...
		classNames = common.unique(common.flatten(classNames)).join(' ');
		if (classNames) {
	...
	function normalizeClassName(className) {
		...
		var shortSymbol = preferences_1.get('bem.shortElementPrefix');
		if (shortSymbol) {
			var re = new RegExp('\\s(' + common.escapeForRegexp(shortSymbol) + '+)', 'g');
			className = className.replace(re, function(str, p1) {
				return ' ' + common.repeatString(getSeparators().element, p1.length);
			});
		}
		
		return common.trim(className);
	}
	...
	function processClassName(name, item) {
		name = transformClassName$3(name, item, 'element');
		name = transformClassName$3(name, item, 'modifier');
...
//				element = item.parent.__bem.element + separators.element + element;
	 ...
	 * transformed
	 */
	function transformClassName$3(name, item, entityType) {
		var separators = getSeparators();
...
//				}
	 ...
	 * @param {Object} profile
	 */
	function process$1(tree, profile) {
		if (tree.name) {
			bemParse(tree, profile);
		}
		
		tree.children.forEach(function(item) {
			process$1(item, profile);
			if (!abbreviation.isSnippet(item) && item.start) {
...
var bem = function(tree, profile) {
		shouldRunHtmlFilter = false;
		tree = process$1(tree, profile);
		// in case 'bem' filter is applied after 'html' filter: run it again
		// to update output
		if (shouldRunHtmlFilter) {
			tree = html(tree, profile);
		}
		
		return tree;
	};

preferences_1.define('filter.commentAfter', 
			'\n<!-- /<%= attr("id", "#") %><%= attr("class", ".") %> -->',
	...
	preferences_1.define('filter.commentBefore', 
			'',
	...
	preferences_1.define('filter.commentTrigger', 'id, class',
	...
	function addComments(node, templateBefore, templateAfter) {
		...
		var trigger = preferences_1.get('filter.commentTrigger');
		if (trigger != '*') {
			var shouldAdd = common.find(trigger.split(','), function(name) {
				return !!node.attribute(common.trim(name));
			});
	...
	function process$2(tree, before, after) {
		tree.children.forEach(function(item) {
			if (abbreviation.isBlock(item)) {
				addComments(item, before, after);
			}
			
			process$2(item, before, after);
		});
			
		return tree;
	}

var comment$2 = function(tree) {
		var templateBefore = template(preferences_1.get('filter.commentBefore'));
		var templateAfter = template(preferences_1.get('filter.commentAfter'));
		
		return process$2(tree, templateBefore, templateAfter);
	};
...
var _escape = function process(tree) {
		tree.children.forEach(function(item) {
...
var singleLine = function process(tree) {
		tree.children.forEach(function(item) {
			if (!abbreviation.isSnippet(item)) {
			...
			// remove newlines 
			item.start = item.start.replace(reNl$2, '');
			item.end = item.end.replace(reNl$2, '');
			item.content = item.content.replace(reNl$2, '');
...
preferences_1.define('filter.trimRegexp', 
		'[\\s|\\u00a0]*[\\d|#|\\-|\*|\\u2022]+\\.?\\s*',
...
var trim = function(tree) {
		var re = new RegExp(preferences_1.get('filter.trimRegexp'));
		return process$3(tree, re);
	};
...
var main = {
		...
		apply: function(tree, filters, profileName) {
			...
			list(filters).forEach(function(filter) {
				var name = common.trim(filter.toLowerCase());
...
var langs = {
	...
	preferences_1.define('lorem.defaultLang', 'en', 
	...
	preferences_1.define('lorem.omitCommonPart', false,
...
var lorem = {
		...
		preprocessor: function(tree) {
			...
			var allowCommon = !preferences_1.get('lorem.omitCommonPart');
			
			/** @param {AbbreviationNode} node */
			tree.findAll(function(node) {
				if (node._name && (match = node._name.match(re))) {
					var wordCound = match[2] || 30;
					var lang = match[1] || preferences_1.get('lorem.defaultLang') || 'en';
...
var outputPlaceholder = '$#';
	...
	function replaceOutputPlaceholders(source, value) {
		...
		ranges.reverse().forEach(function(r) {
			source = common.replaceSubstring(source, value, r);
		});
	...
	function hasOutputPlaceholder(node) {
		...
		return !!common.find(node.attributeList(), function(attr) {
	...
	function insertPastedContent(node, content, overwrite) {
		...
		if (nodesWithPlaceholders.length) {
			...
			if (overwrite) {
				...
				deepest.content = abbreviation.insertChildContent(deepest.content, content);
			}
		}
	}
...
var pastedContent = {
		...
		preprocessor: function(tree, options) {
			if (options.pastedContent) {
				var lines = common.splitByLines(options.pastedContent, true).map(common.trim);
	...
	function resolveNodeNames(tree) {
		tree.children.forEach(function(node) {
			if (node.hasImplicitName() || node.data('forceNameResolving')) {
				node._name = tagName$1.resolve(node.parent.name());
	...
	function matchResources(node, syntax, parser) {
		...
		node.children.slice(0).forEach(function(child) {
			...
			if (elemType == 'snippet') {
				...
				if (curContent) {
					content = abbreviation.insertChildContent(content, curContent);
				}
...
var resourceMatcher = {
		preprocessor: function(tree, options, parser) {
			var syntax = options.syntax || common.defaultSyntax();
		...
		node.children.forEach(function(item) {
			...
			if (typeof defaultAttrValue !== 'undefined') {
				...
				if (attrList.length) {
					...
					var defaultAttr = common.find(attrList, findDefault) 
						|| common.find(attrList, findImplied) 
						|| common.find(attrList, findEmpty);

					if (defaultAttr) {
						var oldVal = item.attribute(defaultAttr.name);
						var newVal = common.replaceUnescapedSymbol(oldVal, '|', defaultAttrValue);
...
preferences_1.define('href.autodetect', true, 
	...
	preferences_1.define('href.urlPattern', '^(?:(?:https?|ftp|file)://|www\\.|ftp\\.)(?:\\([-A-Z0-9+&@#/%=~_|$?!:,.]*\\)|[-A-Z0-9+&@#/%=~_|$?!:,.])*(?:\\([-A-Z0-9+&@#/%=~_|$?!:,.]*\\)|[A-Z0-9+&@#/%=~_|$])', 
	...
	preferences_1.define('href.emailPattern', '^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,5}$', 
...
var href = {
		 ...
		 */
		postprocessor: function(tree, options) {
			if (!preferences_1.get('href.autodetect')) {
				return;
			}

			var reUrl = new RegExp(preferences_1.get('href.urlPattern'), 'i');
			var reEmail = new RegExp(preferences_1.get('href.emailPattern'), 'i');
			var reProto = /^([a-z]+:)?\/\//i;
			...
			tree.findAll(function(item) {
				...
				var pastedContent$$1 = common.trim(pastedContent.pastedContent(item) || options.pastedContent);
				if (pastedContent$$1) {
					if (reUrl.test(pastedContent$$1)) {
						// do we have protocol?
						if (!reProto.test(pastedContent$$1)) {
							pastedContent$$1 = 'http://' + pastedContent$$1;
						}

						item.attribute('href', pastedContent$$1);
					} else if (reEmail.test(pastedContent$$1)) {
						item.attribute('href', 'mailto:' + pastedContent$$1);
					}
				}
			});
		}
...
// pre- and post-processorcs
	...
	AbbreviationNode.prototype = {
		...
		clone: function() {
			...
			node._attributes = this._attributes.map(function(attr) {
				return common.extend({}, attr);
			});
			
			node._data = common.extend({}, this._data);
		...
		replace: function() {
			...
			var items = common.flatten(arguments);
		...
		findAll: function(fn, state) {
			state = common.extend({amount: 0, found: 0}, state || {});
		...
		attribute: function(name, value) {
			...
			return (common.find(this.attributeList(), function(attr) {
				return attr.name == name;
			}) || {}).value;
		},
		...
		valueOf: function() {
			...
			content = abbreviation.insertChildContent(content, innerContent, {
				keepVariable: false
			});
			
			return start + common.padString(content, this.padding) + end;
		},
		...
		hasEmptyChildren: function() {
			return !!common.find(this.children, function(child) {
	...
	function splitAttributes(attrSet) {
		...
		while ((ch = stream.next())) {
			if (ch == ' ') {
				parts.push(common.trim(stream.current()));
				// skip spaces
				while (stream.peek() == ' ') {
					stream.next();
				}
	...
	outputProcessors.push(function(text, node) {
		return common.replaceCounter(text, node.counter, node.maxCount);
	});
	...
	[lorem, resourceMatcher, attributes$1, pastedContent, tagName_1$1, href].forEach(function(mod) {
		if (mod.preprocessor) {
...
var abbreviation$1 = {
		...
		expand: function(abbr, options) {
			...
			if (!options.syntax) {
				options.syntax = common.defaultSyntax();
			}

			var p = profile.get(options.profile, options.syntax);
			tabStops.resetTabstopIndex();
			
			var data = main.extract(abbr);
			var outputTree = this.parse(data[0], options);

			var filtersList = main.composeList(options.syntax, p, data[1]);
			main.apply(outputTree, filtersList, p);
			...
			while (true) {
				...
				if (ch == ']') {
					...
					if (braceCount || textCount) 
						...
						continue;
					else if (!abbreviation$1.isAllowedChar(ch) || (ch == '>' && common.endsWithTag(str.substring(0, curOffset + 1)))) {
						// found stop symbol
						startIndex = curOffset + 1;
						break;
					}
		...
		captureContext: function(editor, pos) {
			...
			if (syntax in allowedSyntaxes) {
				...
				if (tag && tag.type == 'tag') {
					...
					// parse attributes
					var tagTree = xml$1.parse(startTag.range.substring(content));
					if (tagTree) {
...
var editor = {
		...
		unindent: function(editor, text) {
			return common.unindentString(text, this.getCurrentLinePadding(editor));
		},
		...
		getCurrentLinePadding: function(editor) {
			return common.getLinePadding(editor.getCurrentLine());
		},
		...
		normalize: function(text, options) {
			...
			var indent = function(tabs) {
				return common.repeatString(options.indentation, tabs.length);
			};

			var lines = common.splitByLines(text);
...
/**
	 ...
	 * @return {Boolean}
	 */
	function encodeToBase64(editor$$1, imgPath, pos) {
		var editorFile = editor$$1.getFilePath();
		...
		file.locateFile(editorFile, imgPath, function(realImgPath) {
			...
			file.read(realImgPath, function(err, content) {
				...
				b64 = 'data:' + (action.mimeTypes[String(file.getExt(realImgPath))] || defaultMimeType) +
					';base64,' + b64;

				editor$$1.replaceContent('$0' + b64, pos, pos + imgPath.length);
			});
		});
	 ...
	 * @param {Number} pos Caret position where image is located in the editor
	 */
	function decodeFromBase64(editor$$1, filePath, data, pos) {
		// ask user to enter path to file
		filePath = filePath || String(editor$$1.prompt('Enter path to file (absolute or relative)'));
		if (!filePath) {
			return false;
		}
		...
		file.createPath(editorFile, filePath, function(err, absPath) {
			...
			file.save(absPath, base64.decode(content), function(err) {
				...
				editor$$1.replaceContent('$0' + filePath, pos, pos + data.length);
			});
		});
...
var base64_1$1 = {
		 ...
		 * @return {Boolean}
		 */
		encodeDecodeDataUrlAction: function(editor$$1, filepath) {
			var data = String(editor$$1.getSelection());
			var caretPos = editor$$1.getCaretPos();
			var info = editor.outputInfo(editor$$1);
			...
			if (data) {
				if (startsWith('data:', data)) {
					return decodeFromBase64(editor$$1, filepath, data, caretPos);
				} else {
					return encodeToBase64(editor$$1, data, caretPos);
				}
			}
...
var evaluateMath = {
		...
		evaluateMathAction: function(editor) {
			...
			if (!sel.length()) {
				sel = action.findExpressionBounds(editor, function(ch) {
					return common.isNumeric(ch) || chars.indexOf(ch) != -1;
				});
			}
			...
			if (sel && sel.length()) {
				...
				try {
					var result = common.prettifyNumber(math.evaluate(expr));
...
// all directions are expressed in “new style” degrees
	...
	function LinearGradient(dfn) {
		...
		// extract tokens
		var stream = stringStream.create(common.trim(dfn));
		var ch, cur;
		while ((ch = stream.next())) {
			if (stream.peek() == ',') {
	...
	function normalizeSpace(str) {
		return common.trim(str).replace(/\s+/g, ' ');
	}
...
var cssGradient = createCommonjsModule(function (module) {
var cssSyntaxes = ['css', 'less', 'sass', 'scss', 'stylus', 'styl'];
	
	// XXX define preferences
	preferences_1.define('css.gradient.prefixes', 'webkit, moz, o',
			'A comma-separated list of vendor-prefixes for which values should ' 
			+ 'be generated.');
	
	preferences_1.define('css.gradient.oldWebkit', false,
			'Generate gradient definition for old Webkit implementations');
	
	preferences_1.define('css.gradient.omitDefaultDirection', true,
		'Do not output default direction definition in generated gradients.');
	
	preferences_1.define('css.gradient.defaultProperty', 'background-image',
		'When gradient expanded outside CSS value context, it will produce '
			+ 'properties with this name.');
	
	preferences_1.define('css.gradient.fallback', false,
	...
	function resolvePropertyName(name, syntax) {
		...
		if (!snippet && preferences_1.get('css.fuzzySearch')) {
			var minScore = parseFloat(preferences_1.get('css.fuzzySearchMinScore'));
		...
		if (snippet) {
			...
			return css$2.splitSnippet(snippet).name;
		}
	}
	...
	function getGradientPrefixes(type) {
		var prefixes = css$2.vendorPrefixes(type);
		if (!prefixes) {
			// disabled Can I Use, fallback to property list
			prefixes = preferences_1.getArray('css.gradient.prefixes');
		}
	...
	function getPropertiesForGradient(gradients, property) {
		...
		var omitDir = preferences_1.get('css.gradient.omitDefaultDirection');
		
		if (preferences_1.get('css.gradient.fallback') && ~propertyName.toLowerCase().indexOf('background')) {
			props.push({
		...
		getGradientPrefixes('linear-gradient').forEach(function(prefix) {
			var name = css$2.prefixed(propertyName, prefix);
			if (prefix == 'webkit' && preferences_1.get('css.gradient.oldWebkit')) {
				try {
					props.push({
						name: name,
	...
	function insertGradientsIntoCSSValue(gradients, value, options) {
		...
		options = options || {};
		gradients = common.clone(gradients);
		gradients.reverse().forEach(function(item, i) {
			...
			value = common.replaceSubstring(value, str + suffix, item.matchedPart);
		});
	...
	function similarPropertyNames(property) {
		...
		var similarProps = (css$2.vendorPrefixes(property) || []).map(function(prefix) {
	...
	function pasteGradient(property, gradients) {
		...
		var alignVendor = preferences_1.get('css.alignVendor');
		var omitDir = preferences_1.get('css.gradient.omitDefaultDirection');
		...
		if (alignVendor) {
			...
			var valuePads = common.getStringsPads(values.map(function(v) {
				return v.substring(0, v.indexOf('('));
			}));
			
			var namePads = common.getStringsPads(names);
...
module.exports = module.exports = {
		...
		findGradients: function(cssProp) {
			...
			cssProp.valueParts().forEach(function(part) {
				...
				if (linear.isLinearGradient(partValue)) {
					var gradient = linear.parse(partValue);
					if (gradient) {
						gradients.push({
							gradient: gradient,
							matchedPart: part
		...
		gradientsFromCSSProperty: function(css, pos) {
			var cssProp = css$1.propertyFromPosition(css, pos);
			if (cssProp) {
		 ...
		 * return {Boolean}
		 */
		expandAbbreviationHandler: function(editor$$1, syntax, profile) {
			var info = editor.outputInfo(editor$$1, syntax, profile);
			if (!~cssSyntaxes.indexOf(info.syntax)) {
				return false;
			}
			
			// let's see if we are expanding gradient definition
			var caret = editor$$1.getCaretPos();
			...
			if (gradients) {
				...
				if (/[\n\r]/.test(cssProperty.value())) {
					...
					var insertPos = cssProperty.valueRange(true).start + common.last(gradients.gradients).matchedPart.end;
					content = common.replaceSubstring(content, ';', insertPos);
				...
				editor$$1.replaceContent(cssRule.toString(), ruleStart, ruleEnd, true);
				return true;
			}
			
			return this.expandGradientOutsideValue(editor$$1, syntax);
		},
		 ...
		 * @param {String} syntax
		 */
		expandGradientOutsideValue: function(editor$$1, syntax) {
			var propertyName = preferences_1.get('css.gradient.defaultProperty');
			var omitDir = preferences_1.get('css.gradient.omitDefaultDirection');
			...
			var content = String(editor$$1.getContent());
			/** @type Range */
			var lineRange = range.create(editor$$1.getCurrentLineRange());
			...
			if (gradients) {
				...
				var sep = css$2.getSyntaxPreference('valueSeparator', syntax);
				var end = css$2.getSyntaxPreference('propertyEnd', syntax);
				
				if (preferences_1.get('css.alignVendor')) {
					var pads = common.getStringsPads(props.map(function(prop) {
				...
				editor$$1.replaceContent(props.join('\n'), lineRange.start, lineRange.end);
				return true;
			}
...
var cssGradient_1 = cssGradient.findGradients;
var cssGradient_2 = cssGradient.gradientsFromCSSProperty;
var cssGradient_3 = cssGradient.expandAbbreviationHandler;
var cssGradient_4 = cssGradient.expandGradientOutsideValue;
var cssGradient_5 = cssGradient.reflectValueHandler;
...
/**
	 ...
	 * @return {String}
	 */
	function findAbbreviation(editor$$1) {
		var r = range(editor$$1.getSelectionRange());
		var content = String(editor$$1.getContent());
		if (r.length()) {
		...
		var curLine = editor$$1.getCurrentLineRange();
		return action.extractAbbreviation(content.substring(curLine.start, r.start));
	}
	 ...
	 * successfully
	 */
	handlers.add(function(editor$$1, syntax, profile) {
		var caretPos = editor$$1.getSelectionRange().end;
		var abbr = findAbbreviation(editor$$1);
			
		if (abbr) {
			var content = abbreviation$1.expand(abbr, {
				syntax: syntax, 
				profile: profile, 
				contextNode: action.captureContext(editor$$1)
			});
			...
			if (content) {
				...
				if (cssSyntaxes && ~cssSyntaxes.indexOf(syntax)) {
					var curContent = editor$$1.getContent();
					if (curContent.charAt(caretPos) == ';' && content.charAt(content.length - 1) == ';') {
						replaceTo++;
					}
				}

				editor$$1.replaceContent(content, replaceFrom, replaceTo);
				return true;
			}
		}
...
var expandAbbreviation = {
		 ...
		 * @return {Boolean}
		 */
		expandAbbreviationAction: function(editor$$1, syntax, profile) {
		 ...
		 * @return {Boolean}
		 */
		expandAbbreviationWithTabAction: function(editor$$1, syntax, profile) {
			var sel = editor$$1.getSelection();
			var indent = '\t';

			// if something is selected in editor,
			// we should indent the selected content
			if (sel) {
				var selRange = range(editor$$1.getSelectionRange());
				var content = common.padString(sel, indent);
				
				editor$$1.replaceContent(indent + '${0}', editor$$1.getCaretPos());
				var replaceRange = range(editor$$1.getCaretPos(), selRange.length());
				editor$$1.replaceContent(content, replaceRange.start, replaceRange.end, true);
				editor$$1.createSelection(replaceRange.start, replaceRange.start + content.length);
				return true;
			}
	
			// nothing selected, try to expand
			if (!this.expandAbbreviationAction(editor$$1, syntax, profile)) {
				editor$$1.replaceContent(indent, editor$$1.getCaretPos());
			}
		...
		_defaultHandler: function(editor$$1, syntax, profile) {
			var caretPos = editor$$1.getSelectionRange().end;
			var abbr = this.findAbbreviation(editor$$1);
				
			if (abbr) {
				var ctx = action.captureContext(editor$$1);
				var content = abbreviation$1.expand(abbr, syntax, profile, ctx);
				if (content) {
					editor$$1.replaceContent(content, caretPos - abbr.length, caretPos);
					return true;
				}
			}
...
var incrementDecrement = {
		...
		incrementNumber: function(editor, step) {
			...
			var r = action.findExpressionBounds(editor, function(ch, pos, content) {
				if (common.isNumeric(ch))
					return true;
				if (ch == '.') {
					// make sure that next character is numeric too
					if (!common.isNumeric(content.charAt(pos + 1)))
						return false;
			...
			if (r && r.length()) {
				...
				if (!isNaN(num)) {
					num = common.prettifyNumber(num + step);
					...
					if (/^(\-?)0+[1-9]/.test(strNum)) {
						...
						var parts = num.split('.');
						parts[0] = common.zeroPadString(parts[0], intLength(strNum));
...
var xmlSyntaxes = ['html', 'xml', 'xsl'];
	...
	preferences_1.define('css.closeBraceIndentation', '\n',
...
var lineBreaks = {
		 ...
		 * @param {IEmmetEditor} editor Editor instance
		 */
		insertLineBreakAction: function(editor$$1) {
			if (!this.insertLineBreakOnlyAction(editor$$1)) {
				var curPadding = editor.getCurrentLinePadding(editor$$1);
				var content = String(editor$$1.getContent());
				var caretPos = editor$$1.getCaretPos();
				...
				var lineRange = editor$$1.getCurrentLineRange();
				var nextPadding = '';
				...
				if (nextPadding.length > curPadding.length) {
					editor$$1.replaceContent(nl + nextPadding, caretPos, caretPos, true);
				} else {
					editor$$1.replaceContent(nl, caretPos);
				}
			}
		 ...
		 */
		insertLineBreakOnlyAction: function(editor$$1) {
			var info = editor.outputInfo(editor$$1);
			var caretPos = editor$$1.getCaretPos();
			var nl = '\n';
			var pad = '\t';
			...
			if (~xmlSyntaxes.indexOf(info.syntax)) {
				...
				if (tag && !tag.innerRange.length()) {
					editor$$1.replaceContent(nl + pad + common.getCaretPlaceholder() + nl, caretPos);
					return true;
				}
				...
				if (caretPos && content.charAt(caretPos - 1) == '{') {
					var append = preferences_1.get('css.closeBraceIndentation');
					...
					// defining rule set
					var insValue = nl + pad + common.getCaretPlaceholder() + append;
					editor$$1.replaceContent(insValue, caretPos);
					return true;
				}
			}
...
var lastMatch = null;
	...
	function balanceHTML(editor$$1, direction) {
		var info = editor.outputInfo(editor$$1);
		var content = info.content;
		var sel = range(editor$$1.getSelectionRange());
			...
			if (direction == 'in') {
				...
				if (lastMatch.type == 'tag' && !lastMatch.close) {
					...
					if (lastMatch.range.equal(lastMatch.outerRange)) {
						...
						var narrowed = common.narrowToNonSpace(content, lastMatch.innerRange);
		...
		if (lastMatch) {
			...
			if (!lastMatch.range.equal(sel)) {
				editor$$1.createSelection(lastMatch.range.start, lastMatch.range.end);
				return true;
			}
		}
	...
	function rangesForCSSRule(rule, pos) {
		...
		var prop = css$1.propertyFromPosition(rule, pos) || items[0];
		if (prop) {
	...
	function getCSSRanges(content, pos) {
		...
		if (typeof content === 'string') {
			...
			if (ruleRange) {
				rule = css$1.parse(ruleRange.substring(content), {
		...
		return common.unique(ranges, function(item) {
			return item.valueOf();
		});
	}

	function balanceCSS(editor$$1, direction) {
		var info = editor.outputInfo(editor$$1);
		var content = info.content;
		var sel = range(editor$$1.getSelectionRange());
		...
		if (!ranges && sel.length()) {
			...
			try {
				var rule = css$1.parse(sel.substring(info.content), {
					offset: sel.start
				});
		...
		var bestMatch = common.find(ranges, function(r) {
			return r.equal(sel);
		});

		if (!bestMatch) {
			bestMatch = common.find(ranges, function(r) {
		...
		if (bestMatch) {
			editor$$1.createSelection(bestMatch.start, bestMatch.end);
			return true;
		}
...
var balance = {
		 ...
		 * Default is 'out'
		 */
		balance: function(editor$$1, direction) {
			direction = String((direction || 'out').toLowerCase());
			var info = editor.outputInfo(editor$$1);
			if (action.isSupportedCSS(info.syntax)) {
				return balanceCSS(editor$$1, direction);
			}
			
			return balanceHTML(editor$$1, direction);
		},

		balanceInwardAction: function(editor$$1) {
			return this.balance(editor$$1, 'in');
		},

		balanceOutwardAction: function(editor$$1) {
			return this.balance(editor$$1, 'out');	
		},
		 ...
		 * @param {IEmmetEditor} editor
		 */
		goToMatchingPairAction: function(editor$$1) {
			var content = String(editor$$1.getContent());
			var caretPos = editor$$1.getCaretPos();
			...
			if (tag && tag.close) { // exclude unary tags
				if (tag.open.range.inside(caretPos)) {
					editor$$1.setCaretPos(tag.close.range.start);
				} else {
					editor$$1.setCaretPos(tag.open.range.start);
				}
...
var mergeLines = {
		mergeLinesAction: function(editor$$1) {
			var info = editor.outputInfo(editor$$1);
		
			var selection = range(editor$$1.getSelectionRange());
			if (!selection.length()) {
				// find matching tag
				var pair = htmlMatcher.find(info.content, editor$$1.getCaretPos());
				if (pair) {
			...
			if (selection.length()) {
				...
				text = common.escapeText(text);
				editor$$1.replaceContent(text, selection.start, selection.end);
				editor$$1.createSelection(selection.start, selection.start + textLen);
...
var reflectCSSValue = createCommonjsModule(function (module) {
preferences_1.define('css.reflect.oldIEOpacity', false, 'Support IE6/7/8 opacity notation, e.g. <code>filter:alpha(opacity=...)</code>.\
	...
	function doCSSReflection(editor$$1) {
		var outputInfo = editor.outputInfo(editor$$1);
		var caretPos = editor$$1.getCaretPos();
		
		var cssRule = css$1.parseFromPosition(outputInfo.content, caretPos);
		if (!cssRule) return;
	...
	function getReflectedCSSName(name) {
		name = css$1.baseName(name);
		var vendorPrefix = '^(?:\\-\\w+\\-)?', m;
		
		if ((name == 'opacity' || name == 'filter') && preferences_1.get('css.reflect.oldIEOpacity')) {
	...
	function reflectValueParts(name, value) {
		...
		var parts = css$1.findParts(value);

		parts.reverse();
		parts.forEach(function(part) {
			var partValue = part.substring(value).replace(reVendor, '');
			var prefixes = css$2.vendorPrefixes(partValue);
			if (prefixes) {
				...
				value = common.replaceSubstring(value, partValue, part);
			}
		});
	...
	function getReflectedValue(curName, curValue, refName, refValue) {
		curName = css$1.baseName(curName);
		refName = css$1.baseName(refName);
		
		if (curName == 'opacity' && refName == 'filter') {
			return refValue.replace(/opacity=[^)]*/i, 'opacity=' + Math.floor(parseFloat(curValue) * 100));
		} else if (curName == 'filter' && refName == 'opacity') {
			var m = curValue.match(/opacity=([^)]*)/i);
			return m ? common.prettifyNumber(parseInt(m[1], 10) / 100) : refValue;
		}
	...
	module = module || {};
	module.exports = {
		reflectCSSValueAction: function(editor$$1) {
			if (editor$$1.getSyntax() != 'css') {
				return false;
			}

			return action.compoundUpdate(editor$$1, doCSSReflection(editor$$1));
		},
...
var reflectCSSValue_1 = reflectCSSValue.reflectCSSValueAction;
var reflectCSSValue_2 = reflectCSSValue._defaultHandler;
var reflectCSSValue_3 = reflectCSSValue.addHandler;
var reflectCSSValue_4 = reflectCSSValue.removeHandler;

var removeTag = {
		removeTagAction: function(editor$$1) {
			var info = editor.outputInfo(editor$$1);
			
			// search for tag
			var tag = htmlMatcher.tag(info.content, editor$$1.getCaretPos());
			if (tag) {
				if (!tag.close) {
					// simply remove unary tag
					editor$$1.replaceContent(common.getCaretPlaceholder(), tag.range.start, tag.range.end);
				} else {
					// remove tag and its newlines
					/** @type Range */
					var tagContentRange = common.narrowToNonSpace(info.content, tag.innerRange);
					/** @type Range */
					var startLineBounds = common.findNewlineBounds(info.content, tagContentRange.start);
					var startLinePad = common.getLinePadding(startLineBounds.substring(info.content));
					var tagContent = tagContentRange.substring(info.content);
					
					tagContent = common.unindentString(tagContent, startLinePad);
					editor$$1.replaceContent(common.getCaretPlaceholder() + common.escapeText(tagContent), tag.outerRange.start, tag.outerRange.end);
				}
...
var startTag$1 = /^<([\w\:\-]+)((?:\s+[\w\-:]+(?:\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|[^>\s]+))?)*)\s*(\/?)>/;
	 ...
	 */
	function findItem(editor$$1, isBackward, extractFn, rangeFn) {
		var content = editor.outputInfo(editor$$1).content;
		...
		while (searchPos >= 0 && searchPos < contentLength && --loop > 0) {
			if ( (itemRange = extractFn(content, searchPos, isBackward)) ) {
				...
				if (rng) {
					editor$$1.createSelection(rng.start, rng.end);
					return true;
				} else {
	 ...
	 * @param {IEmmetEditor} editor
	 */
	function findNextHTMLItem(editor$$1) {
		var isFirst = true;
		return findItem(editor$$1, false, function(content, searchPos){
			if (isFirst) {
				isFirst = false;
	 ...
	 * @param {IEmmetEditor} editor
	 */
	function findPrevHTMLItem(editor$$1) {
		return findItem(editor$$1, true, getOpeningTagFromPosition, function (tag, offset, selRange) {
	...
	function makePossibleRangesHTML(source, tokens, offset) {
		...
		tokens.forEach(function(tok) {
			switch (tok.type) {
				...
				case 'string':
					...
					if (isQuote$1(attrValue.charAt(0)))
						attrValueRange.start++;
					
					if (isQuote$1(attrValue.charAt(attrValue.length - 1)))
						attrValueRange.end--;
		...
		// remove duplicates
		return common.unique(result, function(item) {
	...
	function getRangeForHTMLItem(tag, offset, selRange, isBackward) {
		var ranges = makePossibleRangesHTML(tag, xml.parse(tag), offset);
		
		if (isBackward)
			ranges.reverse();
		
		// try to find selected range
		var curRange = common.find(ranges, function(r) {
		...
		if (isBackward)
			// search backward
			return common.find(ranges, function(r) {
	...
	function getOpeningTagFromPosition(html, pos) {
		var m;
		if (html.charAt(pos) == '<' && (m = html.substring(pos, html.length).match(startTag$1))) {
			return range(pos, m[0]);
		}
	}
	
	function isQuote$1(ch) {
		return ch == '"' || ch == "'";
	}
	...
	function makePossibleRangesCSS(property) {
		...
		property.valueParts().forEach(function(r) {
			...
			if (stream.match(/^[\w\-]+\(/, true)) {
				...
				// find parts
				css$1.findParts(fnBody).forEach(function(part) {
	...
	function matchedRangeForCSSProperty(rule, selRange, isBackward) {
		...
		var r = common.find(ranges, function(item) {
		...
		var test = 
		r = common.find(ranges, isBackward 
	...
	function findNextCSSItem(editor$$1) {
		return findItem(editor$$1, false, cssSections.locateRule.bind(cssSections), getRangeForNextItemInCSS);
	}
	
	function findPrevCSSItem(editor$$1) {
		return findItem(editor$$1, true, cssSections.locateRule.bind(cssSections), getRangeForPrevItemInCSS);
	}
...
var selectItem = {
		selectNextItemAction: function(editor$$1) {
			if (action.isSupportedCSS(editor$$1.getSyntax())) {
				return findNextCSSItem(editor$$1);
			} else {
				return findNextHTMLItem(editor$$1);
			}
		},

		selectPreviousItemAction: function(editor$$1) {
			if (action.isSupportedCSS(editor$$1.getSyntax())) {
				return findPrevCSSItem(editor$$1);
			} else {
				return findPrevHTMLItem(editor$$1);
			}
		}
	};
...
/**
	 ...
	 * @param {Object} tag
	 */
	function joinTag(editor$$1, profile$$1, tag) {
		// empty closing slash is a nonsense for this action
		var slash = profile$$1.selfClosing() || ' /';
		...
		content = common.escapeText(content);
		editor$$1.replaceContent(content, tag.outerRange.start, tag.outerRange.end);
		editor$$1.setCaretPos(caretPos);
		return true;
	}
	
	function splitTag(editor$$1, profile$$1, tag) {
		var caretPos = editor$$1.getCaretPos();
		
		// define tag content depending on profile
		var tagContent = (profile$$1.tag_nl === true) ? '\n\t\n' : '';
...
var splitJoinTag = {
		splitJoinTagAction: function(editor$$1, profileName) {
			var info = editor.outputInfo(editor$$1, null, profileName);
			...
			if (tag) {
				return tag.close 
					? joinTag(editor$$1, curProfile, tag) 
					: splitTag(editor$$1, curProfile, tag);
			}
...
var range$2 = range;
	 ...
	 */
	function toggleHTMLComment(editor$$1) {
		/** @type Range */
		var r = range$2(editor$$1.getSelectionRange());
		var info = editor.outputInfo(editor$$1);
		...
		return genericCommentToggle(editor$$1, '<!--', '-->', r);
	}
	 ...
	 * @return {Boolean} Returns <code>true</code> if comment was toggled
	 */
	function toggleCSSComment(editor$$1) {
		/** @type Range */
		var rng = range$2(editor$$1.getSelectionRange());
		var info = editor.outputInfo(editor$$1);
			
		if (!rng.length()) {
			// no selection, try to get current rule
			/** @type CSSRule */
			var rule = css$1.parseFromPosition(info.content, editor$$1.getCaretPos());
			if (rule) {
				var property = cssItemFromPosition(rule, editor$$1.getCaretPos());
				rng = property 
					? property.range(true) 
					: range$2(rule.nameRange(true).start, rule.source);
			}
		}
		
		if (!rng.length()) {
			// still no selection, get current line
			rng = range$2(editor$$1.getCurrentLineRange());
			common.narrowToNonSpace(info.content, rng);
		}
		
		return genericCommentToggle(editor$$1, '/*', '*/', rng);
	}
	...
	function cssItemFromPosition(rule, absPos) {
		...
		var reSafeChar = /^[\s\n\r]/;
		return common.find(rule.list(), function(item) {
	...
	function searchComment(text, from, startToken, endToken) {
		...
		return (commentStart != -1 && commentEnd != -1) 
			? range$2(commentStart, commentEnd - commentStart) 
			: null;
	}
	 ...
	 * @return {Boolean}
	 */
	function genericCommentToggle(editor$$1, commentStart, commentEnd, range$$1) {
		var content = editor.outputInfo(editor$$1).content;
		var caretPos = editor$$1.getCaretPos();
		var newContent = null;
		...
		function removeComment(str) {
			return str
				.replace(new RegExp('^' + common.escapeForRegexp(commentStart) + '\\s*'), function(str){
					caretPos -= str.length;
					return '';
				}).replace(new RegExp('\\s*' + common.escapeForRegexp(commentEnd) + '$'), '');
		}
		
		// first, we need to make sure that this substring is not inside 
		// comment
		var commentRange = searchComment(content, caretPos, commentStart, commentEnd);
		if (commentRange && commentRange.overlap(range$$1)) {
			// we're inside comment, remove it
			range$$1 = commentRange;
			newContent = removeComment(range$$1.substring(content));
		} else {
			// should add comment
			// make sure that there's no comment inside selection
			newContent = commentStart + ' ' +
				range$$1.substring(content)
					.replace(new RegExp(common.escapeForRegexp(commentStart) + '\\s*|\\s*' + common.escapeForRegexp(commentEnd), 'g'), '') +
				' ' + commentEnd;
		...
		if (newContent !== null) {
			newContent = common.escapeText(newContent);
			editor$$1.setCaretPos(range$$1.start);
			editor$$1.replaceContent(editor.unindent(editor$$1, newContent), range$$1.start, range$$1.end);
			editor$$1.setCaretPos(caretPos);
			return true;
		}
...
var toggleComment = {
		 ...
		 * @param {IEmmetEditor} editor
		 */
		toggleCommentAction: function(editor$$1) {
			var info = editor.outputInfo(editor$$1);
			if (action.isSupportedCSS(info.syntax)) {
			...
			var cssSyntaxes = preferences_1.getArray('css.syntaxes');
			if (~cssSyntaxes.indexOf(info.syntax)) {
				return toggleCSSComment(editor$$1);
			}
			
			return toggleHTMLComment(editor$$1);
		}
	};
...
/**
	 ...
	 * @param {IEmmetEditor} editor
	 */
	function updateImageSizeHTML(editor$$1) {
		var offset = editor$$1.getCaretPos();

		// find tag from current caret position
		var info = editor.outputInfo(editor$$1);
		var xmlElem = xml$1.parseFromPosition(info.content, offset, true);
		if (xmlElem && (xmlElem.name() || '').toLowerCase() == 'img') {
			getImageSizeForSource(editor$$1, xmlElem.value('src'), function(size) {
				if (size) {
					var compoundData = xmlElem.range(true);
					xmlElem.value('width', size.width);
					xmlElem.value('height', size.height, xmlElem.indexOf('width') + 1);

					action.compoundUpdate(editor$$1, common.extend(compoundData, {
	 ...
	 * @param {IEmmetEditor} editor
	 */
	function updateImageSizeCSS(editor$$1) {
		var offset = editor$$1.getCaretPos();

		// find tag from current caret position
		var info = editor.outputInfo(editor$$1);
		var cssRule = css$1.parseFromPosition(info.content, offset, true);
		if (cssRule) {
			// check if there is property with image under caret
			var prop = cssRule.itemFromPosition(offset, true), m;
			if (prop && (m = /url\((["']?)(.+?)\1\)/i.exec(prop.value() || ''))) {
				getImageSizeForSource(editor$$1, m[2], function(size) {
					if (size) {
	 ...
	 */
	function getImageSizeForSource(editor$$1, src, callback) {
		var fileContent;
		if (src) {
			// check if it is data:url
			if (/^data:/.test(src)) {
				fileContent = base64.decode( src.replace(/^data\:.+?;.+?,/, '') );
				return callback(action.getImageSize(fileContent));
			}

			var filePath = editor$$1.getFilePath();
			file.locateFile(filePath, src, function(absPath) {
				...
				file.read(absPath, function(err, content) {
					...
					callback(action.getImageSize(content));
				});
			});
		}
	}
...
var updateImageSize = {
		updateImageSizeAction: function(editor$$1) {
			// this action will definitely won’t work in SASS dialect,
			// but may work in SCSS or LESS
			if (action.isSupportedCSS(editor$$1.getSyntax())) {
				updateImageSizeCSS(editor$$1);
			} else {
				updateImageSizeHTML(editor$$1);
			}
...
var wrapWithAbbreviation = {
		 ...
		 * @param {String} profile Output profile name (html, xml, xhtml)
		 */
		wrapWithAbbreviationAction: function(editor$$1, abbr, syntax, profile) {
			var info = editor.outputInfo(editor$$1, syntax, profile);
			abbr = abbr || editor$$1.prompt("Enter abbreviation");
			...
			if (!r.length()) {
				...
				r = common.narrowToNonSpace(info.content, match.range);
			}
			
			var newContent = common.escapeText(r.substring(info.content));
			var result = abbreviation$1.expand(abbr, {
				pastedContent: editor.unindent(editor$$1, newContent),
				syntax: info.syntax,
			...
			if (result) {
				editor$$1.replaceContent(result, r.start, r.end);
				return true;
			}
...
function updateAttributes(tag, abbrNode, ix) {
		...
		var r = function(str) {
			return common.replaceCounter(str, abbrNode.counter);
		};
...
var updateTag = {
		 ...
		 */
		updateTagAction: function(editor$$1, abbr) {
			abbr = abbr || editor$$1.prompt("Enter abbreviation");
			...
			var content = editor$$1.getContent();
			var ctx = action.captureContext(editor$$1);
			...
			if (tag.name() != ctx.name && ctx.match.close) {
				editor$$1.replaceContent('</' + tag.name() + '>', ctx.match.close.range.start, ctx.match.close.range.end, true);
			}

			editor$$1.replaceContent(tag.source, ctx.match.open.range.start, ctx.match.open.range.end, true);
			return true;
		},
		...
		getUpdatedTag: function(abbr, ctx, content, options) {
			...
			var tree = abbreviation$1.parse(abbr, options || {});
			...
			var tag = xml$1.parse(ctx.match.open.range.substring(content), {
...
// all registered actions
	...
	var actionModules = {
		...
		reflectCSSValue: reflectCSSValue,
		removeTag: removeTag,
	...
	function humanizeActionName(name) {
		return common.trim(name.charAt(0).toUpperCase() 
...
var main$1 = {
		...
		run: function(name, args) {
			if (!Array.isArray(args)) {
				args = common.toArray(arguments, 1);
			}
			...
			this.getList().forEach(function(action) {
				...
				if (action.options.label) {
					...
					while ((menuName = parts.shift())) {
						submenu = common.find(ctx, function(item) {
		...
		getActionNameForMenuTitle: function(title, menu) {
			return common.find(menu || this.getMenu(), function(val) {
				if (val.type == 'action') {
...
var sliceFn = Array.prototype.slice;
	 ...
	 * @return {Object}
	 */
	function normalizeProfile(profile$$1) {
		if (typeof profile$$1 === 'object') {
			if ('indent' in profile$$1) {
				profile$$1.indent = !!profile$$1.indent;
			}

			if ('self_closing_tag' in profile$$1) {
				if (typeof profile$$1.self_closing_tag === 'number') {
					profile$$1.self_closing_tag = !!profile$$1.self_closing_tag;
				}
			}
		}
...
var emmet = {
		 ...
		 * @return {String}
		 */
		expandAbbreviation: function(abbr, syntax, profile$$1, contextNode) {
			return abbreviation$1.expand(abbr, {
				syntax: syntax,
				profile: profile$$1,
		 ...
		 */
		run: function(name) {
			return main$1.run.apply(main$1, sliceFn.call(arguments, 0));
		},
		...
		loadExtensions: function(fileList) {
			...
			var next = function() {
				if (fileList.length) {
							...
							case 'json':
								...
								if (/^snippets/.test(fileName)) {
									if (fileName === 'snippets') {
										...
										payload.snippets = common.deepMerge(payload.snippets || {}, content);
									}
								} else {
					...
					// complete
					if (userSnippets) {
						payload.snippets = common.deepMerge(payload.snippets || {}, userSnippets);
					}
		...
		loadPreferences: function(data) {
			preferences_1.load(common.parseJSON(data));
		},
		...
		loadSnippets: function(data) {
			...
			resources.setVocabulary(common.deepMerge(userData, data), 'user');
		},
		...
		loadSystemSnippets: function(data) {
			resources.setVocabulary(common.parseJSON(data), 'system');
		},
		...
		loadCIU: function(data) {
			caniuse.load(common.parseJSON(data));
		},
		...
		loadSyntaxProfiles: function(profiles) {
			profiles = common.parseJSON(profiles);
			var snippets = {};
...
// this is an entry file to be built by rollup
